...
allvars.crosshairbool = false
allvars.crosshairspin = false
allvars.crosshairsize = 20
allvars.crosshairthickness = 2
allvars.crosshairgap = 5
allvars.crosshairspinspeed = 1
allvars.crosshairimageid = ""
allvars.crosshaircolor = Color3.new(0, 1, 0) -- Default green
...
getgenv().aimignoreparts = {}


-- Create FOV Circle
getgenv().aimfovcircle = Drawing.new("Circle")
...
getgenv().knockCheckEnabled = true
getgenv().nojumptilt = false
getgenv().a1table = nil
getgenv().joindetect = true
getgenv().leavedetect = true
                    ...
                    if bot and bot:IsA("Model") and bot:FindFirstChild("Humanoid") and bot.Humanoid.Health > 0 then
                        ...
                        if potroot and potroot.Position and localplayer.Character and localplayer.Character.PrimaryPart then
                            ...
                            if success and spoint then
                                ...
                                if getgenv().aimfovcircle and dist <= getgenv().aimfovcircle.Radius and dist < cdist and betweendist < (allvars.aimdistance or math.huge) and betweendistSTUDS < ammodistance and isonscreen(potroot) then
...
function runhitmark(v140)
    ...
    local success, err = pcall(function()
        ...
        hit_img.Image = "rbxassetid://9720078113"
...
require(game.Players.LocalPlayer.PlayerScripts.PlayerModule.CameraModule.TransparencyController).Update = function(a1, a2) -- transparency = allvars.camthirdp and 1 or 0
    ...
    local setto = 0
    if allvars.camthirdp == false or getgenv().thirdpshow == false then
        setto = 1
    end
...
runs.Heartbeat:Connect(function(delta) --silent aim + trigger bot fast cycle
    ...
    if allvars.aimtrigger and aimtarget and not hasWallBetween(startPos, endPos, aimtarget) and fpsrequired and getgenv().a1table then --trigger bot
...
-- Add loot ESP toggle to your UI
...
espUI2:AddToggle('Loot Highlight Enabled', {
    Text = 'Loot Highlight',
    Default = false,
...
espUI2:AddLabel('Loot Color'):AddColorPicker('LootColor', {
...
espUI2:AddLabel('Highlight Color'):AddColorPicker('LootHighlightColor', {
...
WorldStuff:AddToggle("Shadows", {
    ...
    Callback = function(value)
        ...
        if value ~= originalGlobalShadows then
            -- Auto-lock shadows when changed from default
            ...
            -- Remove protection when back to original
            if shadowConnection then
                shadowConnection:Disconnect()
...
originalFire = hookfunction(game.ReplicatedStorage.Remotes.FireProjectile.InvokeServer, function(...)
    ...
    if Tracers.Enabled and args[2] then -- args[2] is direction vector
        ...
        if gunModel and gunModel:FindFirstChild("ItemRoot") then
            ...
            local endPos = startPos + (args[2] * 5000) 
...
local function handleMagazine(a1)
    local mag = a1.weapon.Attachments:FindFirstChild("Magazine")
    if mag then
        local lastAmmo = mag:GetChildren()[#mag:GetChildren()]
        if lastAmmo then
            a1.Bullets = lastAmmo.ItemProperties:GetAttribute("LoadedAmmo")
            a1.BulletsList = {}
            
            for _, ammo in ipairs(lastAmmo.ItemProperties.LoadedAmmo:GetChildren()) do
                a1.BulletsList[tonumber(ammo.Name)] = {
                    AmmoType = ammo:GetAttribute("AmmoType"),
                    Amount = ammo:GetAttribute("Amount")
                }
            end
        end
    end
...
local function updateTransparency(item, folderName)
    local folder = item:FindFirstChild(folderName)
    if folder then
        for _, child in ipairs(folder:GetChildren()) do
            child.Transparency = 1
        end
        local visible = folder:FindFirstChild(ammoTypeValue)
        if visible then visible.Transparency = 0 end
    end
end
...
runs.RenderStepped:Connect(function(delta) -- global fast
    ...
    -- Subtle nofall method (anti-cheat friendly)
local humstate = localplayer.Character.Humanoid:GetState()
local currentVelocityY = localplayer.Character.HumanoidRootPart.AssemblyLinearVelocity.Y

if allvars.nofall then
    -- Only activate when actually falling dangerously
    if (humstate == Enum.HumanoidStateType.FallingDown or humstate == Enum.HumanoidStateType.Freefall) and currentVelocityY < -40 then
        -- Simple state change - mimics natural landing
        localplayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Landed)
    end
    
    -- Instant fall (only when safe and not being tracked by AC)
    if allvars.instafall and aimresolver == false and currentVelocityY < -50 then
        ...
        if fray then
            -- More natural teleport - preserves horizontal velocity
            local currentPos = localplayer.Character.HumanoidRootPart.CFrame
            local groundPos = fray.Position + Vector3.new(0, 3, 0)
            
            localplayer.Character.HumanoidRootPart.CFrame = CFrame.new(groundPos, currentPos.LookVector)
            
            -- Don't completely zero velocity - just reduce Y component
            local vel = localplayer.Character.HumanoidRootPart.AssemblyLinearVelocity
            localplayer.Character.HumanoidRootPart.AssemblyLinearVelocity = Vector3.new(vel.X, -5, vel.Z)
        end
    end
end
    ...
    if allvars.aimdynamicfov then -- fov changer
        getgenv().aimfovcircle.Radius = allvars.aimfov * (80 / wcamera.FieldOfView )
    else
        getgenv().aimfovcircle.Radius = allvars.aimfov
    end

    getgenv().aimfovcircle.Position = Vector2.new(wcamera.ViewportSize.X / 2, wcamera.ViewportSize.Y / 2)
    getgenv().aimfovcircle.Color = allvars.aimfovcolor
    if scgui then
